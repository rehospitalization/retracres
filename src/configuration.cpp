...
void Configuration::setWidth(int w) {
    ...
    const std::wstring recommended = L"-" + width + L".000000";
    ...
    this->ini.SetValue(section, L"LastRecommendedScreenWidth", recommended.c_str());
}

void Configuration::setHeight(int h) {
    std::wstring height = std::to_wstring(h);
    const std::wstring recommended = L"-" + height + L".000000";
    ...
    this->ini.SetValue(section, L"LastRecommendedScreenHeight", recommended.c_str());
}
...
void Configuration::setWindowMode(int m) {
    ...
    // Borderless/Windowed should use the configured resolution instead of
    // silently falling back to the desktop size when possible.
    this->ini.SetValue(section, L"bUseDesktopResolution", L"False");
}
